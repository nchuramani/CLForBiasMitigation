...
def RafdbTask(train_dataset, val_dataset, task_num):
    train_datasets = {}
    val_datasets = {}
    task_output_space = {}   
    for i in range(1, task_num + 1):
        name = str(i)
        index_train = find_indexes(train_dataset, i-1)
        index_val = find_indexes(val_dataset, i-1)
        train_dataset2 = Subset(train_dataset, index_train)
        val_dataset2 = Subset(val_dataset, index_val)
        train_datasets[name] = AppendName(train_dataset2, name)
        val_datasets[name] = AppendName(val_dataset2, name)
        task_output_space[name] = task_num
    return train_datasets, val_datasets, task_output_space

def find_indexes(dataset, label_name):
    indexes = []
    for i in range(len(dataset)):
        if(dataset[i][1] == int(label_name)):
            indexes.append(i)
    return indexes
